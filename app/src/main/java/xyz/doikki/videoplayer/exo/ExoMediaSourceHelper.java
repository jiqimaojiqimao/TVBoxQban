...
import com.google.androidx.media3.exoplayer.extractor.ts.DefaultSubtitleParserFactory;
...
private static synchronized ExtractorsFactory getExtractorsFactory() {
    return new DefaultExtractorsFactory() {
        ...
        public Extractor[] createExtractors() {
            // 获取默认的提取器列表
            Extractor[] defaultExtractors = super.createExtractors();
            
            // 创建 M2tsExtractor 实例
            M2tsExtractor m2tsExtractor = new M2tsExtractor(
                new DefaultSubtitleParserFactory()  // 需要导入这个类
            );
            
            // 将 M2tsExtractor 添加到提取器列表中
            Extractor[] extractors = new Extractor[defaultExtractors.length + 1];
            System.arraycopy(defaultExtractors, 0, extractors, 0, defaultExtractors.length);
            extractors[defaultExtractors.length] = m2tsExtractor;
